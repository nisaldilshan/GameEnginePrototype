...
namespace Hazel {
	...
	void Renderer2D::Init()
	{
		...
		// Set first texture slot to be 0
	...
	void Renderer2D::DrawQuad(const glm::vec3& position, const glm::vec2& size, const glm::vec4& color)
	{
		...
		s_Data.QuadVertexBufferPtr = s_Data.QuadVertexBufferBase;
		s_Data.TextureSlots[0]->Bind(0); // bind white texture
#endif

		glm::mat4 transform = glm::translate(glm::mat4(1.0f), position) * glm::scale(glm::mat4(1.0f), { size.x, size.y, 1.0f });

		const float textureIndex = 0.0f; // index of White Texture
	...
	void Renderer2D::DrawQuad(const glm::vec3& position, const glm::vec2& size, const std::shared_ptr<Texture2D>& texture, float tilingFactor, const glm::vec4& tintColor)
	{
		...
		//s_Data.TextureSlots[0]->Bind(0); // it doesn't matter if we bind this or not, because its only multiplying by 1
		texture->Bind(1);
#else
